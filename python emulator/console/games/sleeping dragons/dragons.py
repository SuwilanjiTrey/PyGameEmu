...
def draw():
	...
	if game_over:
		screen.draw.text("GAME OVER!!!", color=FONT_COLOR, center=CENTER, fontsize = 60)
	elif game_complete:
...
def draw_lairs(lairs_go_here):
	for lair in lairs_go_here:
		lair["dragon"].draw()
...
def subtract_life():
	global reset_lives, reset_required, game_over
	lives -= 1
	if lives == 0:
		game_over = True
	reset_required = False
